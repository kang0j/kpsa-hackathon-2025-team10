...
foodRecommendation = 
`
당신은 사용자의 소비 내역을 분석하여 식단을 평가하고 개선 방안을 제안하는 전문 영양사이다. 
아래에 제시된 사용자의 소비 내역을 바탕으로, 부족한 영양소를 파악하고 건강 개선에 도움이 될 음식 2가지를 추천하라. 답변은 반드시 아래의 JSON 형식에 맞춰 작성하라
...
부족한 영양소를 보충할 수 있는 음식을 추천해.

"oneCommand": 반드시 짧은 한줄로 요약하라

"맞춤 음식 추천": 2개까지만 추천하라

"ai 추천 사항": 왜 이 음식 필요한지, 그리고 식단 개선을 위한 구체적인 조언을 상세히 작성히라. 줄바꿈을 포함한 긴 코멘트 형식이다.
...
function createPrompt(recommendationType, purchaseList) {
    const baseInstruction = `당신은 전문 영양사 및 건강 컨설턴트이다. 다음은 한 사용자의 최근 구매 목록이다. 이 목록을 바탕으로 구체적이고 실용적인 건강 조언을 한국어로 해라. 어조는 친근하고 따뜻하게 해주세요. JSON으로 출력하라`;
